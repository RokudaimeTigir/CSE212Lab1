...
public class Menu {
	...
	public static void main(String[] args) {
		...
		play.add(new Play("Macbeth", "20:00", "22.11.2020", 2));
		...
		while (true) {
			...
			System.out.println("5. Display all Content");
			...
			if (choice.equals("1")) {
				...
				if (flag) {
					...
					if (choice.equals("1")) {
						...
						while (true) {
							...
							choice = s.nextLine();
							try {
								int dummy = Integer.parseInt(choice.charAt(3) + "");
								break;
							} catch (Exception e) {
								System.err.println("Please enter a valid promotion number!");
							}
				...
				Iterator<Play> iteratorPlay = play.iterator();
				while (iteratorPlay.hasNext()) {
					display.add(iteratorPlay.next());
				}
				Iterator<Ticket> iteratorTicket = ticket.iterator();
				while (iteratorTicket.hasNext()) {
					display.add(iteratorTicket.next());
				}

				for (Displayable current : display) {
					current.otherDisplay();
				}
